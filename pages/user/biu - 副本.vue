			...
			<view class="body-data">
				<view v-show="currIdx===0">
					...
					<view class="u-f-a jianjie-item">
						<image style="height: 42rpx;width: 42rpx;" mode="heightFix" :src="iconUrls[0]"></image>
						<view class="jianjie-text" @longpress="copy(model.nation+'-'+model.province)">
							{{model.nation+'-'+model.province}}
						</view>
					</view>
					<view class="u-f-a jianjie-item">
						<image style="height: 42rpx;width: 42rpx;" mode="heightFix" :src="iconUrls[1]"></image>
						<view class="jianjie-text" @longpress="copy(model.sex)"> {{model.sex}}</view>
					</view>
					<view class="u-f-a jianjie-item">
						<image style="height: 42rpx;width: 42rpx;" mode="heightFix" :src="iconUrls[2]"></image>
						<view class="jianjie-text" @longpress="copy(model.age)"> {{model.age}}</view>
					</view>
					<view class="u-f-a jianjie-item" style="border: none!important;">
						<image style="height: 42rpx;width: 42rpx;" mode="heightFix"
							:src="txfsUrls[model.communicate]"></image>
						<view class="jianjie-text" @longpress="copy(model.communicate.tag)"> {{model.communicate.tag}}
						</view>
					</view>
...
<script>
	...
	export default {
		...
		onLoad(event) {
			...
			this.mingpian.friend = _detail.friend;

			this.initData();

			console.info("this.mingpian:::", this.mingpian);
			//
		...
		methods: {
			...
			initData() {
				...
				that.$http.post(_url, _params).then(res => {
					...
					if (_result.code === 200) {
						that.mingpian = _result.data;
						console.info("_result.data:5.30::", that.mingpian);
